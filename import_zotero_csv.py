...
示例:
    ...
    python scripts/import_zotero_csv.py data/我的文库.csv  --replace-missing-pdf --base-url http://192.168.1.185:5001 --token we-love-control-and-network
"""
...
def upload_pdf_for_existing_doc(base_url: str, doc_id: int, file_path: str, token: str = None) -> dict | None:
    """为已有文档上传/替换 PDF 文件。"""
    try:
        headers = {}
        if token:
        ...
        with httpx.Client(timeout=120) as client:
            ...
            if resp.status_code == 200:
                ...
                print(f"  替换 PDF 失败: {resp.status_code} - {resp.text[:200]}")
    except Exception as e:
        print(f"  替换 PDF 失败: {e}")
    return None
...
def main():
    ...
    parser.add_argument("--replace-missing-pdf", action="store_true",
                        help="当 DOI 匹配的文档缺少 PDF 文件时，上传 PDF 替换")
        ...
        if not args.no_skip:
            ...
            if existing:
                ...
                if has_pdf:
                    ...
                    stats["duplicate"] += 1
                    continue
                elif args.replace_missing_pdf:
                    print(f"  已存在 (ID: {doc_id}) 但缺少 PDF，正在替换...")
                    if args.dry_run:
                        print(f"  [试运行] 将替换 PDF")
                        stats["uploaded"] += 1
                        continue
                    result = upload_pdf_for_existing_doc(args.base_url, doc_id, newest_file, token)
                    if result:
                        print(f"  已替换 PDF: ID={doc_id}")
                        stats["uploaded"] += 1
                    else:
                        stats["failed"] += 1
                    continue
                else:
                    print(f"  跳过: 已存在但缺少 PDF (ID: {doc_id})，使用 --replace-missing-pdf 替换")
                    stats["duplicate"] += 1
                    continue
